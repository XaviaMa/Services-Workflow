...
if [ "$(echo $DATA | cut -f 1 -d " ")" == "200" ]; then
        ...
        COMMITTER=$(echo $JSON | awk -F 'committer' '{print $2}' - | awk -F 'username' '{print $NF}' - | cut -f 3 -d "\"") 2>&1 3>&1 > /dev/null
        ...
        COMMIT=$(echo $JSON | awk -F 'commits' '{print $NF}' - | awk -F 'id' '{print $NF}' - | cut -f 3 -d "\"")
